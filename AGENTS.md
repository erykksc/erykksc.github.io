...
## Commands
- Use `npm run dev` for the Astro dev server and `npm run build` for the main verification step.
- There are no repo scripts for lint, test, typecheck, or format; do not invent them. `npm run build` is the only configured production check.
- The project requires Node `>=22.12.0` and uses `package-lock.json`, so prefer npm commands.
- `npm run build` currently emits an Astro sitemap warning because `astro.config.mjs` does not define `site`; this warning is known and unrelated to most changes.
